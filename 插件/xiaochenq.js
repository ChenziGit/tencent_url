...
function formatMusicItem(song) {
    ...
    if (Array.isArray(song.singer)) {
        singerStr = song.singer.map(s => s.name || "").join(", ");
    ...
    if (song.album && song.album.name) {
        ...
        albumStr = song.albumname;
    } else if (typeof song.album === "string") {
        albumStr = song.album;
    }

    let picUrl = song.pic || "";
    if (!picUrl && song.album && song.album.mid) {
        picUrl = `https://y.qq.com/music/photo_new/T002R800x800M000${song.album.mid}.jpg?max_age=2592000`;
    }

    return {
        id: String(song.songmid || song.id),
        title: song.songname || song.title || song.name || "未知歌曲",
        artist: singerStr,
        album: albumStr,
        artwork: picUrl, // 封面如果有的话
